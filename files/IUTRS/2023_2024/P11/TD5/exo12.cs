...
class Exercice12
{
	...
	{
		// tableau vide
		int n = 0;	// aucune valeur dans le tableau
		int n_max = 10;	// tableau de 10 entiers
		int[] tab = new int[n_max];
		AfficherChaine("Tableau vide de " + n_max + " cases.");
		AfficherTableau(tab);
		
		// insertion de 10 valeurs demandées à l'utilisateur avec sous-tableau trié
		while (n < n_max)
		{
			int entier_a_inserer = LireEntier("Entrez un entier à insérer dans le tableau : ");
			int indice_insertion = IndiceInsertionPartiellementTrie(tab, n, entier_a_inserer);
			AfficherChaine("La valeur " + entier_a_inserer + " est à insérer à l'indice " + indice_insertion + ".");
			InsertionPartiellementTrie(tab, n, indice_insertion, entier_a_inserer);
			AfficherTableau(tab);
			n++;
		}
	}
