...
class Exercice14
{
	...
	// Fonction : calcule la longueur de la monotonie croissante à partir d'un indice donné dans un tableau d'entiers
	// Paramètres :
	// 	- tab (int[]): tableau d'entiers
	//	- debut (int): indice de début de la monotonie
	// Retourne : longueur (int): la longueur de la monotonie croissante débutant en `debut` dans `tab`
	public static int LongueurMonotonieCroissante(int[] tab, int debut)
	{
		int longueur = 1;
		int i = debut + 1;
		// tant que la monotonie est croissante et que l'on est pas arrivé en fin de tableau
		while (i < tab.Length && tab[i] >= tab[i-1])
		{
			i++;
			longueur++;
		}
	...
	// Fonction : recherche la plus longue monotonie croissante dans un tableau d'entiers
	// Paramètres :
	// 	- tab (int[]): tableau d'entiers
	// Retourne : rien, effectue des affichages
	public static void PlusLongueMonotonieCroissante(int[] tab)
	{
		int longueur_pl = 0;
		int debut_pl = 0;
		int indice_recherche = 0;
		while (indice_recherche < tab.Length)
		{
			int longueur_monotonie = LongueurMonotonieCroissante(tab, indice_recherche);
			if (longueur_monotonie > longueur_pl)
			{
				debut_pl = indice_recherche;
				longueur_pl = longueur_monotonie;
			}
			indice_recherche += longueur_monotonie;
		}
		AfficherChaine("La plus longue monotonie croissante débute en " + debut_pl + " et sa longueur est de " + longueur_pl + "."); 
	}
	...
	static void Main()
	{
		// tableau aléatoire
		int[] tab = new int[25];
		RemplirAleatoire(tab, 0, 100);
		AfficherChaine("Tableau alétoire.");
		AfficherTableau(tab);
		PlusLongueMonotonieCroissante(tab);
	}
}
